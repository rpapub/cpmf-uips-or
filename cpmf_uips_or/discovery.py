...
def _traverse(
    ...
    if node_type == "App":
        ...
        if content_path.exists():
            element_content = element_adapter.parse_content(content_path)
            if element_content:
                scope_vars = element_adapter.extract_variables(element_content.scope_selector)
                selector_vars = element_adapter.extract_variables(element_content.full_selector)

                # Get screenshot dimensions if available
                screenshot_width = None
                screenshot_height = None
                if element_content.screenshot and screenshots_dir:
                    img_size = _get_image_size(screenshots_dir / element_content.screenshot)
                    if img_size:
                        screenshot_width, screenshot_height = img_size
                ...
                elements.append(
                    ElementEntry(
                        ...
                        search_steps=element_content.search_steps,
                        scope_selector=element_content.scope_selector,
                        full_selector=element_content.full_selector,
                        browser_url=element_content.browser_url,
                        element_type=element_content.element_type,
                        activity_type=element_content.activity_type,
                        reference=reference,
                        descriptor_version=element_content.version,
                        content_path=content_path,
                        parent_ref=meta.parent_ref if meta else None,
                        screenshot=element_content.screenshot,
                        screenshot_width=screenshot_width,
                        screenshot_height=screenshot_height,
                        fuzzy_selector=element_content.fuzzy_selector,
                        has_image=element_content.has_image,
                        has_cv=element_content.has_cv,
                        cv_type=element_content.cv_type,
                        visibility=element_content.visibility,
                        wait_for_ready=element_content.wait_for_ready,
                        scope_variables=scope_vars,
                        selector_variables=selector_vars,
                        declared_variables=element_content.variables,
                        created=meta.created if meta else None,
                        updated=meta.updated if meta else None,
                        created_by=meta.created_by if meta else None,
                        updated_by=meta.updated_by if meta else None,
...
def audit_all(
    ...
    for elem in elements:
        if elem.is_parameterized:
            result.parameterized_count += 1
            for var in elem.scope_variables + elem.selector_variables:
                result.variables_in_use.add(var)
        else:
            result.hardcoded_count += 1

        version = elem.descriptor_version
        result.version_counts[version] = result.version_counts.get(version, 0) + 1

        if elem.descriptor_version not in {"V6"}:
            result.issues.append(
                f"Unsupported Element version {elem.descriptor_version}: {elem.full_path}"
            )
