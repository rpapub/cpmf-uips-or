...
from .__version__ import __schema_version__
    ...
    if output_format == OutputFormat.tree:
        ...
        if elements:
            ...
            for elem in elements:
                typer.echo(f"{elem.full_path} [{elem.descriptor_version}]")
                typer.echo(f"  SearchSteps:   {elem.search_steps}")
                typer.echo(f"  Type:          {elem.element_type} / {elem.activity_type}")
                typer.echo(f"  Visibility:    {elem.visibility or '(not set)'}")
                typer.echo(f"  WaitForReady:  {elem.wait_for_ready or '(not set)'}")
                typer.echo(f"  Scope:         {_truncate(elem.scope_selector, 80)}")
                typer.echo(f"  Selector:      {_truncate(elem.full_selector, 80)}")

                # Show additional selector types based on SearchSteps
                if "FuzzySelector" in elem.search_steps:
                    if elem.fuzzy_selector:
                        typer.echo(f"  Fuzzy:         {_truncate(elem.fuzzy_selector, 80)}")
                    else:
                        typer.echo("  Fuzzy:         (configured but empty)")
                if "Image" in elem.search_steps:
                    typer.echo(f"  Image:         {'present' if elem.has_image else '(configured but empty)'}")
                if "CV" in elem.search_steps:
                    if elem.has_cv:
                        typer.echo(f"  CV:            {elem.cv_type or 'present'}")
                    else:
                        typer.echo("  CV:            (configured but empty)")

                # Show parameterization status
                if elem.scope_variables:
                    typer.echo(f"  Scope vars:    {', '.join(elem.scope_variables)}")
                if elem.selector_variables:
                    typer.echo(f"  Selector vars: {', '.join(elem.selector_variables)}")
                typer.echo()
...
def replace(
    project: Path = typer.Argument(..., help="Path to project.json"),
    config: Path | None = typer.Option(None, "--config", help="Path to rules TOML file"),
    type_filter: TypeFilter = typer.Option(TypeFilter.all, "--type", help="Filter by type"),
...
def _apply_preview(preview: ReplacePreview) -> bool:
    ...
    if is_screen:
        ...
        assert isinstance(preview.entry, ElementEntry)
...
def orphans(
    ...
    orphaned_names_sorted = sorted(orphaned_names)
    orphaned_sizes = [all_files[name] for name in orphaned_names_sorted]
    orphaned: list[_OrphanItem] = [
        {"file": name, "size": size}
        for name, size in zip(orphaned_names_sorted, orphaned_sizes, strict=False)
    ]
    total_bytes = sum(orphaned_sizes)
