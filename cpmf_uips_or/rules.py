...
def preview_replacements(
    ...
    # Build lookup dict for cascade support
    ...
    for elem in elements:
        ...
        # Check version support
        if not element_adapter.is_version_supported(elem.descriptor_version):
            if not force:
                warnings.append(
                    f"Skipping {elem.full_path}: unsupported version {elem.descriptor_version}"
                )
                continue

        # Track which target types have been matched (allows one rule per target type)
        elem_matched_targets: set[RuleTarget] = set()
        for rule in rules:
            # Check scope selector
            if RuleTarget.ELEMENT_SCOPE not in elem_matched_targets:
                matches, attr, old_attr_value = _match_element_rule(elem, rule, "scope")
                if matches and old_attr_value:
                    old_selector, new_selector = _compute_element_selector_preview(
                        elem.scope_selector, attr or "", old_attr_value, rule
                    )
                    previews.append(
                        ReplacePreview(
                            entry=elem,
                    ...
                    elem_matched_targets.add(RuleTarget.ELEMENT_SCOPE)
                    continue

            # Check full selector
            if RuleTarget.ELEMENT_SELECTOR not in elem_matched_targets:
                matches, attr, old_attr_value = _match_element_rule(elem, rule, "selector")
                if matches and old_attr_value:
                    old_selector, new_selector = _compute_element_selector_preview(
                        elem.full_selector, attr or "", old_attr_value, rule
                    )
                    previews.append(
                        ReplacePreview(
                            entry=elem,
                    ...
                    elem_matched_targets.add(RuleTarget.ELEMENT_SELECTOR)
